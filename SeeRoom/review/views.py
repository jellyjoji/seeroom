...
class makeReviewAtBuilding(APIView):
    ...
    def post(self, request, pk):
        ...
        review_data = {'contents': request.data["reviewList.contents"],
                       'userId': request.user.id, 'buildingId': self.kwargs['pk'], 'recommend': 0}
        ...
        if build_data_serialize.is_valid():
            ...
            cleanAvg = (build_data_serialize.cleanAvg*(count-1)+(build_data_serialize.smellScore +
                        build_data_serialize.bugScore + build_data_serialize.moldScore)/3)/count
            noiseAvg = (build_data_serialize.noiseAvg*(count-1)+(build_data_serialize.floorNoiseScore +
                        build_data_serialize.externalNoiseScore + build_data_serialize.internalNoiseScore)/3)/count
            locationsAvg = (build_data_serialize.locationsAvg*(count-1)+(build_data_serialize.parkingScore +
                            build_data_serialize.managementScore + build_data_serialize.constructionScore)/3)/count
            safeAvg = (build_data_serialize.safeAvg*(count-1)+((1.25 if build_data_serialize.femaleOnly else 0.0) + (1.25 if build_data_serialize.cctv else 0.0) +
                       (1.25 if build_data_serialize.courierBox else 0.0) + (1.25 if build_data_serialize.elevator else 0.0)))/count
            last_build = Building.objects.get(id=self.kwargs['pk'])
            last_data = {"cleanAvg": cleanAvg, "noiseAvg": noiseAvg,
                         "locationsAvg": locationsAvg, "safeAvg": safeAvg}
